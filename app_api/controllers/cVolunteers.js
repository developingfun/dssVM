		   		...
		   		.findById(req.params.id ,
						function(err,volunteer) {
							...
							if (!volunteer)	{
								...
								console.log("** app_api/cVolunteers: activity pushed save volunteer"+activity._id);
								volunteer.save(function(err1,v) {
									if (err1) {
										console.log("** error saving volunteer "+err1);
										sendJsonResponse(res, 500, {"message" : err1});
								...
								console.log("** app_api/cVolunteers: end function "+activity._id);
...
module.exports.volunteerRecordOutTime = function (req,res) {
	console.log("** app_api/cVolunteers : call record in time");
	if (req.params && req.params.id && req.params.activityId) {
			console.log("** app_api/cVolunteers : update activity with outTime "+req.params.activityId);
			// add to volunteer activity
			Volunteer
		   		.findById(req.params.id ,
						function(err,volunteer) {
							...
							if (!volunteer)	{
								...
								console.log("** app_api/cVolunteers: volunteer found .. pull activity .. udate with "+req.body.outTime);
								volunteer.activities.id(req.params.activityId)
													.set({'outTime':req.body.outTime,
														  'childrenAffected':req.body.childrenAffected,
														  'status':'Attended'
														  }
														 );
								volunteer.markModified('activities');
